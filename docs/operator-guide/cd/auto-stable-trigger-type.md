...
* **Autodeploy**: Automatically triggers the deployment pipeline as soon as a new application version is built, ensuring the latest version is deployed.
...
The diagram below illustrates how the **Auto-stable** trigger type works:
  ...
  ![Application build](../../assets/operator-guide/auto-stable-trigger-type-scheme.png "Application build")
...
* **Stable**: The latest successfully deployed application. Application image gets the **Stable** tag when new application image passes project's quality gates. Only one application image can be stable at the same time.
