...
This integration allows Kubernetes pods to assume IAM roles directly, enabling fine-grained access to AWS resources without needing long-lived credentials. While the deployment process is streamlined for most users, the platform also supports advanced configurations for custom permissions and role management, ensuring flexibility for more complex scenarios.

## Prerequisites

To start using this approach, you need to have OIDC (OpenID Connect) already configured for your EKS cluster. This setup allows Kubernetes service accounts to securely assume IAM roles. For your convenience, please follow our documentation and use the preconfigured deployment code for clusters available in our repository:
[KubeRocketCI Terraform AWS Platform](https://github.com/KubeRocketCI/terraform-aws-platform). This ensures a seamless setup, integrating OIDC with minimal effort.
...
- This service account obtains temporary credentials through IRSA, which are associated with the CD-Pipeline-Operator-IRSA role.
- CD-Pipeline-Operator-IRSA can then assume the CD-Pipeline-Operator-Stage-Access role in AWS Account B.
- CD-Pipeline-Operator-Stage-Access configures the environment (Stage) by creating namespaces, generating service accounts, copying secrets, and preparing for deployment.

### Required IAM Roles, and polices for KRCI

Trust policy initial IRSA role that the service account assumes:
<details>
<summary><b>View: CD-Pipeline-Operator-IRSA (AWS Account A)</b></summary>
```json
{
    "Version": "2012-10-17",
    "Statement": [
...
<details>
<summary><b>View: CDPipeline-Operator-CrossAccountAssumeRole (AWS Account A)</b></summary>
```json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Action": "sts:AssumeRole",
      "Resource": "arn:aws:iam::<AWS_ACCOUNT_B_ID>:role/<CD-Pipeline-Operator-Stage-Access>"
    }
  ]
}
```
...
Trust policy сontrol access to Account B resources:

<details>
<summary><b>View: CD-Pipeline-Operator-Stage-Access (AWS Account B)</b></summary>
```json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/<CD-Pipeline-Operator-IRSA>"
      },
      "Action": "sts:AssumeRole"
    },
    {
...
<details>
<summary><b>View: CDPipeline-Operator-EKSDeployment (AWS Account B)</b></summary>
```json
{
  "Version": "2012-10-17",
  "Statement": [
...
### Required IAM Roles, and polices for ArgoCD

<details>
<summary><b>View: ArgoCD-IRSA-Role (AWS Account A)</b></summary>
    ...
    "Statement": [
        {
            "Effect": "Allow",
            "Principal": {
                "Federated": "arn:aws:iam::<AWS_ACCOUNT_A_ID>:oidc-provider/oidc.eks.<AWS_REGION>.amazonaws.com/id/<OIDC_PROVIDER_ID>"
            },
            "Action": "sts:AssumeRoleWithWebIdentity",
            "Condition": {
                "StringEquals": {
...
<details>
<summary><b>View: ArgoCD-CrossAccount-Access (AWS Account A)</b></summary>
```json
{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
            "Action": "sts:AssumeRole",
            "Resource": "arn:aws:iam::<AWS_ACCOUNT_B_ID>:role/<ArgoCD-Operator-Stage-Access>"
        }
    ]
}
```
...
<details>
<summary><b>View: ArgoCD-Stage-Access (AWS Account B)</b></summary>
```json
{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
            "Principal": {
                "AWS": "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/<ArgoCD-Operator-IRS>"
            },
            "Action": "sts:AssumeRole"
        },
        {
            "Effect": "Allow",
            "Principal": {
                "Federated": "arn:aws:iam::<AWS_ACCOUNT_B_ID>:oidc-provider/oidc.eks.<AWS_REGION>.amazonaws.com/id/<OIDC_PROVIDER_ID>"
            },
            "Action": "sts:AssumeRoleWithWebIdentity",
            "Condition": {
                "StringLike": {
                    "oidc.eks.<AWS_REGION>.amazonaws.com/id/<OIDC_PROVIDER_ID>:sub": "system:serviceaccount:*"
                }
            }
        }
    ]
...
<details>
<summary><b>View: ArgoCD-EKS-Deployment (AWS Account B)</b></summary>
```json
{
    "Version": "2012-10-17",
    "Statement": [
...
<Tabs
  ...
  kubectl patch serviceaccount edp-cd-pipeline-operator -n krci \
    -p '{"metadata": {"annotations": {"eks.amazonaws.com/role-arn": "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/CD-Pipeline-Operator-IRSA"}}}'
  ```
  ...
  metadata:
    annotations:
      eks.amazonaws.com/role-arn: "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/CD-Pipeline-Operator-IRSA"
...
### Add annotations to argocd service accounts (Account A)
...
<Tabs
  ...
  kubectl patch serviceaccount argocd-application-controller -n argocd \
    -p '{"metadata": {"annotations": {"eks.amazonaws.com/role-arn": "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/ArgoCD-IRSA-Role"}}}'
  ```
  ...
  metadata:
    annotations:
      eks.amazonaws.com/role-arn: "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/ArgoCD-IRSA-Role"
...
After applying annotations to service accounts, it is necessary to restart the corresponding deployments to ensure new pods are created with the updated IAM roles configuration. Use the following commands:
...
data:
  mapRoles: |
    - groups:
      ...
      rolearn: "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/CD-Pipeline-Operator-IRSA"
      username: "eksadminrole"
    - groups:
      - "system:masters"
      rolearn: "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/ArgoCD-IRSA-Role>"
      username: "arn:aws:iam::<AWS_ACCOUNT_A_ID>:role/ArgoCD-IRSA-Role"
```
</details>
...
<Tabs
  ...
  * **Role ARN** : arn:aws:iam::\<AWS_ACCOUNT_A_ID\>:role/CD-Pipeline-Operator-IRSA
  ...
  data:
    config: >-
      {
        "server": "https://EXAMPLED539D4633E53DE1B71EXAMPLE.gr7.eu-central-1.eks.amazonaws.com",
        "awsAuthConfig": {
