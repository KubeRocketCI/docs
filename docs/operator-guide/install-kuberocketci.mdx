...
Inspect the main steps to install KubeRocketCI. Please check the [Prerequisites Overview](prerequisites.md) page before starting the installation. Also, to authenticate each of the release artifacts, please refer to the [Verification of KubeRocketCI Artifacts](artifacts-verification.md) guide.
...
1. (Optional) KubeRocketCI manages secrets via `External Secret Operator` to integrate with a variety of tools. For more information about the secrets being used and how they are utilized, please refer to the [External Secrets Operator Integration](./external-secrets-operator-integration.md) guide.

2. (Optional) Deploy and configure Keycloak to enable Single Sign-On approach. To see the details on how to configure Keycloak correctly, please refer to the [Install Keycloak](./advanced-installation/keycloak.md#configuration) page.
...
5. KubeRocketCI can be integrated with the following version control systems:
    ...
    * [Gerrit](https://gerrit-review.googlesource.com/Documentation/)
    ...
    For more details on how to integrate KubeRocketCI with GitLab or GitHub, please refer to the [Integrate GitHub/GitLab with Tekton](./import-strategy-tekton.md) page.
...
6. Configure Container Registry.
    ...
    For more details on integrating these container registries with KubeRocketCI, refer to the following documentation: [Container Registries Integration Guide](./container-registries.md) and [Manage Container Registries](../user-guide/manage-container-registries.md)

    For example, to enable [Harbor](container-registry-harbor-integration-tekton-ci.md) as a registry storage, use the values below:
