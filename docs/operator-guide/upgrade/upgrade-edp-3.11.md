...
3. Replace `edp-config` ConfigMap for Tekton Custom Tasks:
...
4. Update `pipelineUrl` parameter for Tekton Custom Pipelines:
...
5. (Optional) Add yamllint config to the GitOps repository:
...
6. To upgrade KubeRocketCI to the v3.11, run the following command:

    :::note
    To verify the installation, it is possible to test the deployment before applying it to the cluster with the `--dry-run` key:
    `helm upgrade krci epamedp/edp-install -n krci --values values.yaml --version=3.11.1 --dry-run`
    :::

    ```bash
    helm upgrade krci epamedp/edp-install -n krci --values values.yaml --version=3.11.1
    ```

## Versioning Type Changes

Starting from version 3.11, versioning type `edp` was renamed to `semver`. Due to this change, it is necessary to update the existing CodebaseBranch resources with the new Tekton pipelines names. Also, the `edp` versioning type should be replaced with `semver` for the existing Codebase resources that were created with the `edp` versioning type.

### Update CodebaseBranch Resources

:::warning
It is highly recommended to update the CodebaseBranch resources manually, to avoid any unexpected behavior using the bash script.
:::

In version 3.11, Tekton build pipelines were renamed to follow the `semver` versioning type. Due to this change, it is necessary to update the existing CodebaseBranch resources with the new Tekton build pipelines names. For example:
...
### Update Codebase Resources

:::warning
It is highly recommended to update the Codebase resources manually, to avoid any unexpected behavior using the bash script.
:::
