...
During platform's continuous work, various resources are created. Some resources can't be deleted completely, resulting in causing issues, such as creating duplicate resource, storage overflow, etc. This page describes complete resource deletion in KubeRocketCI from application to deployment flow.

## Prerequisites

To follow the guidelines below, ensure you have a deployment flow with applications you no longer need and ready to delete.
...
Many tools are involved in the codebase management process. So it is essential to clean them up as well.
...
KubeRocketCI uses Nexus as a registry for storing application artifacts. If your Nexus instance is installed using our [add-ons](https://github.com/epam/edp-cluster-add-ons/tree/main), your unused images will be deleted in 30 days. To clean up unnecessary container images, follow the steps below:
