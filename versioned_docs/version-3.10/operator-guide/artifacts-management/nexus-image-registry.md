...
1. Navigate to **nexus** section within the KubeRocketCI Cluster Add-Ons repository and align [values.yaml](https://github.com/epam/edp-cluster-add-ons/blob/main/clusters/core/addons/nexus/values.yaml):

2. Enable **nexus** and **nexus-operator** in [Add-Ons](https://github.com/epam/edp-cluster-add-ons/blob/main/clusters/core/apps/values.yaml) repository:
...
With [Add-ons approach](https://github.com/epam/edp-cluster-add-ons/blob/main/clusters/core) `nexus-operator` create the necessary **Service Account** `ci.user`, blob stores, repository, scripts automatically. Below is a comprehensive guide on how to create all resources manually.
...
## Configuration Nexus Repository Manager with [Add-ons approach](https://github.com/epam/edp-cluster-add-ons/blob/main/clusters/core)
  ...
  You can provision secrets using manifests, the KubeRocketCI Portal, or the External Secrets Operator.
...
3. Create repository. Open the Nexus UI and navigate to **Server administration and configuration** -> **Repository** -> **Repositories**, click **+ Create repository**, select **docker (hosted)**, fill the following fields and click **Save** button:
    ...
    * **Blob store name**: `<blob-store-name>`;
