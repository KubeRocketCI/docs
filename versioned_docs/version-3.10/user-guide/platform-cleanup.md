...
During the lifecycle of your projects, various resources are created within the KubeRocketCI platform. There may come a time when you need to clean up resources that are no longer necessary. This could be due to various reasons such as deciding to move applications out of KubeRocketCI management, or compliance with specific regulations or standards necessitating the move of the application to a different environment that meets those requirements. Besides, removing applications without cleanup might cause issues when creating a resource with the same name. All these cases could serve as a reason for platform cleanup.
...
To delete a [deployment flow](./manage-environments.md), follow the steps below:
...
[Codebase Operator](https://github.com/epam/edp-codebase-operator) provisions the repository in the Version Control System (VCS). Below are links to official documentation for deleting code repositories:
