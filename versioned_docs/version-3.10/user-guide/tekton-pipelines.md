...
The KubeRocketCI platform leverages the Tekton stack to run CI/CD pipelines, providing access to all Tekton features out of the box. KubeRocketCI comes pre-configured with various [Tekton resources](https://github.com/epam/edp-tekton/tree/master/charts/pipelines-library) such as pipelines, tasks, triggers, and other components. This documentation outlines how to interact with Tekton resources within the KubeRocketCI platform, covering common use cases, pipeline configuration, and execution processes.

## Pipeline Types in the Platform

Before running pipelines, ensure that the project is onboarded to the KubeRocketCI platform, and the relevant branches are also configured. KubeRocketCI supports four primary pipeline types:
...
To create a pipeline, you need to define and apply Tekton resources such as tasks, pipelines, and triggers in cluster.

KubeRocketCI includes a library component pipelines for managing Tekton resources declaratively. This library integrates with ArgoCD, enabling version control and automatic synchronization of pipeline definitions.
...
4. Check/apply changes in ArgoCD.
...
The TriggerTemplate defines parameters (e.g., service account name, timeout) and references the deploy pipeline in its pipelineRef
field.
...
2. Use the **Run Again** button in the KubeRocketCI UI (if a PipelineRun exists):
...
4. Use the Trigger Build PipelineRun button in the KubeRocketCI UI within the branches section:
...
1. Use **Configure Deploy** and **Start Deploy** buttons in the KubeRocketCI UI:
...
Trigger the clean pipeline using the **Clean** button in the KubeRocketCI UI within the appropriate stage section.
