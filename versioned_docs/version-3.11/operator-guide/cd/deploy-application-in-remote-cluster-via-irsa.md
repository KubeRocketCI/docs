...
- **Using Terraform**: This approach is based on using [terraform-aws-platform](https://github.com/KubeRocketCI/terraform-aws-platform) repository, which provides a set of Terraform modules for managing AWS resources, including IAM roles.
...
In case of using the [terraform-aws-platform](https://github.com/KubeRocketCI/terraform-aws-platform) repository for creating and managing AWS EKS clusters, the required IAM roles can be created directly through the Terraform configuration specified in the repository.
...
To create the required IAM roles in Account B using Terraform, follow the steps below:

1. Clone the forked [terraform-aws-platform](https://github.com/KubeRocketCI/terraform-aws-platform) repository.
2. Navigate to the `eks` directory.
3. Uncomment the following lines in the `irsa.tf` file:

    <details>
      <summary><b>CD Pipeline Operator</b></summary>

    Uncomment the following lines to create the `AWSIRSA_{cluster_name}_CDPipelineAgent` IAM role and Access Entry in Account B:
    ```hcl title="irsa.tf"
   ...
   Uncomment the following lines to create the `AWSIRSA_{cluster_name}_ArgoCDAgentAccess` IAM role and Access Entry in Account B:
    ```hcl title="irsa.tf"
...
4. Update the `terraform.tfvars` configuration file by specifying the following variables:
...
5. Run the following commands to create the AWS resources:
...
:::note
The required Access Entries objects for the IAM roles can be also created using the [terraform-aws-platform](https://github.com/KubeRocketCI/terraform-aws-platform/blob/master/eks/irsa.tf) repository.
:::
