...
|-------------------------------|:------------------:|:------------------:|:------------------:|:-------------------------------------:|:-------------------------------------:|
| `${platform}-oidc-admins`     | :white_check_mark: | :white_check_mark: | :white_check_mark: |          :white_check_mark:           |          :white_check_mark:           |
| `${platform}-oidc-developers` | :white_check_mark: | :white_check_mark: | :white_check_mark: |                :x:                    |          :white_check_mark:           |
| `${platform}-oidc-viewers`    | :white_check_mark: |        :x:         |        :x:         |                :x:                    |                :x:                    |
...
:::note
  The permissions in the table above apply to the **platform namespace**. In environment namespaces the `${platform}-oidc-admins` and `${platform}-oidc-developers` groups are bound to the `admin` ClusterRole and therefore have full access to every resource listed, including Secrets, Roles and Role Bindings, while `${platform}-oidc-viewers` has no access at all. See [Sensitive Capabilities](#sensitive-capabilities).
:::
...
| --------------------------------------------- | :----------: | :----------: | :----------------: | :----------------: | :----------------: |
| Read Secrets                                  |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Create or modify Secrets                      |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Exec into a pod (`pods/exec`)                 |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Port-forward to a pod (`pods/portforward`)    |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Create ServiceAccount tokens                  |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Create Roles and Role Bindings                |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Delete workloads                              |     :x:      |     :x:      |        :x:         | :white_check_mark: | :white_check_mark: |
| Read build logs (`pods/log`, Tekton Results)  | :white_check_mark: | :x:    | :white_check_mark: | :white_check_mark: | :white_check_mark: |
| Approve or reject ApprovalTasks               |     :x:      |     :x:      | :white_check_mark: |        n/a         | :white_check_mark: |
...
:::note
  KubeRocketCI provides an [aggregated](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#aggregated-clusterroles) ClusterRole `edp-aggregate-view-${platform}` with the permissions to view the KubeRocketCI [custom resources](../../api/overview.md). It carries the `rbac.authorization.k8s.io/aggregate-to-view: "true"` label, so its rules are merged into the built-in `view` ClusterRole and are inherited by the viewer and developer groups alike.
:::
