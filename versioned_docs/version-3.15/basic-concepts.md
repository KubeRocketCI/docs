...
|**Artifacts Versioning**|The platform supports two approaches for Artifacts versioning: <br />- `default` (BRANCH-[DATETIME])<br />- `SEMVER` (MAJOR.MINOR.PATCH-BUILD_ID), which follows [SemVer](https://semver.org/). <br />You can also create custom versioning by implementing the `get-version` CI step.|
...
|**CLI Access**|The [krci CLI](./operator-guide/advanced-installation/krci-cli.md) exposes projects, deployments, environments, and security findings from a terminal, using the same identity as the Portal. The same command that answers a question by hand also runs unchanged in a pipeline, on a schedule, or as a tool surface for an AI coding agent.|
