...
|Argo CD Diff Preview|**ALPHA, GitLab only.** Renders an Argo CD manifest diff between a GitOps repository Merge Request and its target branch, and posts it as a single, continuously-updated MR comment. Enabling it requires setting both `pipelines.argocdDiffPreview.enabled: true` in the edp-tekton `pipelines-library` chart and enabling this Add-On in the App-of-Apps values with `createNamespace: false`, since the `pipelines-library` chart already owns the `argocd-diff-preview` namespace. If the Add-On is absent or unhealthy, the pipeline step skips silently and the pipeline stays green.|false|
