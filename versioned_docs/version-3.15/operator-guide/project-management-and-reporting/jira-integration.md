...
:::note
  As of KubeRocketCI 3.15, the build-pipeline automation described below that populated Jira tickets' Fix Versions, Components, and Labels fields no longer runs. The push-to-jira Task and the creation of `JiraIssueMetadata` custom resources were removed from build (non-review) Tekton pipelines. The `JiraServer` CR, the **Integrate with Jira server** codebase setting, and the underlying `Codebase.spec.jiraServer`, `ticketNamePattern`, and `jiraIssueMetadataPayload` fields described below are unchanged - only the build-pipeline automation that consumed them was removed.
:::
...
Previously, upon successful configuration of the Jira integration, tickets were enriched with additional metadata by the build pipeline:
...
:::note
  As of KubeRocketCI 3.15, build (non-review) Tekton pipelines no longer run the push-to-jira Task and no longer create `JiraIssueMetadata` custom resources, so this automatic metadata population no longer happens. If you are running an earlier version where this automation is still in place and metadata is not appearing within a Jira ticket, it's advisable to inspect the status field of the `JiraIssueMetadata` custom resources located in the platform namespace (e.g, `krci`). The codebase-operator typically removes this resource after post-processing, it may persist in case of reconciliation issues, remaining within the namespace.
:::
