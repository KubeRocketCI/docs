...
:::note
  Starting with KubeRocketCI 3.15, the **create codebase** wizard checks a duplicate project name or a Git repository path already onboarded by another `Codebase` inline while you are typing, naming the conflicting project so you can resolve it before submitting. The raw Kubernetes 409 error described in the **Name already exist** row above remains as a fallback for conflicts that are not caught inline (for example, a codebase created by another user after the wizard was opened).
:::
