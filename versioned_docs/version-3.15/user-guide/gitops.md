...
## Preview Argo CD Diff on Merge Requests

:::note
This feature is ALPHA and available for **GitLab** GitOps repositories only.
:::

When enabled, the GitOps review pipeline renders the Argo CD manifest diff between a merge request and its target branch, and posts it as a single Merge Request comment that is kept updated on subsequent pipeline runs.

Enabling the feature requires both of the following:

* `pipelines.argocdDiffPreview.enabled: true` in the `edp-tekton` `pipelines-library` Helm chart values.
* Installing and enabling the `argocd-diff-preview` cluster addon. Since the `edp-tekton` chart owns the addon's namespace, the addon must be installed with `createNamespace: false`.

The diff preview is best-effort at runtime: if the `argocd-diff-preview` addon is unhealthy or not installed, the review pipeline skips the diff preview step silently.
