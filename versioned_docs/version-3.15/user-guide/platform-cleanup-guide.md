...
:::note
The platform rejects deletion of a codebase that is still referenced by a deployment. If a CDPipeline lists the codebase in `spec.applications` or `spec.applicationsToPromote`, lists one of its branches in `spec.inputDockerStreams`, or a Stage quality gate names it as `autotestName`, deleting the codebase is denied at admission with a 403 error naming the referencing CDPipeline or Stage. This applies to every client, including `kubectl`, GitOps, and the Portal, not only the Portal UI. This is why it is recommended to delete deployments before codebases: the deployment reference to the codebase must be removed first.
:::
