...
### HTTPS Requirement for Tool Integration Links

The Quick Link automatically generated when you configure a tool integration (Argo CD,
SonarQube, Nexus, DefectDojo, Dependency-Track, Jira, or the chat assistant) on its
**Configuration** page must be an `https://` URL:

* The integration's configuration form rejects the URL on save if it does not start with
  `https://`, so a non-HTTPS URL can never be persisted for these links.
* If a stored URL is ever non-HTTPS, the link is rendered as plain, non-clickable text on
  the integration's configuration card, and the button that opens it in a new tab is
  disabled.

This requirement does not apply to Quick Links you add manually via **+ Add Link**
(described above) or via the Helm chart — those still accept both `http://` and `https://`
URLs.
