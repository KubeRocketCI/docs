...
<iframe width="560" height="315" src="https://www.youtube.com/embed/1DYKYpWOQP0" title="KubeRocketCI: Deep Dive into CI/CD Pipelines" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen="allowfullscreen"></iframe>
</div>
...
:::note
For GitHub, by default only pull requests and `/recheck` or `/ok-to-test` comments from authors with an `OWNER`, `MEMBER`, or `COLLABORATOR` `author_association` trigger the review pipeline. This is controlled by the `githubAcl.enabled` (`true` by default) and `githubAcl.allowedAssociations` (default `[OWNER, MEMBER, COLLABORATOR]`) values in the `edp-tekton` `pipelines-library` Helm chart. The legacy `githubOwners` filter is now deprecated and disabled by default, since enabling it previously broke `/recheck` for everyone.
:::
